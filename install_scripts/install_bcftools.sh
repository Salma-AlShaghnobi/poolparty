...
archive_file="bcftools-1.5.tar.bz2"

# Full path to the archive file
archive_path="$packages_dir/$archive_file"

# First, try to install bcftools using apt
...
if [[ -f "$archive_path" ]]; then
    ...
    extracted_dir="$packages_dir/bcftools-1.5"
    ...
    if [[ -d "$extracted_dir" ]]; then
        ...
        echo "Creating symlink for bcftools..."
